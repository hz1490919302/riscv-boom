...
class LSUDMemIO(implicit p: Parameters, edge: TLEdgeOut) extends BoomBundle()(p)
{
  ...
  val st_risk_table = Output(Vec(numIntPhysRegs,Bool()))
  val st_fp_risk_table = Output(Vec(numFpPhysRegs,Bool()))
...
class LSUCoreIO(implicit p: Parameters) extends BoomBundle()(p)
{
  ...
  val stq_addr_valid = Output(Vec(numStqEntries,Bool()))
  val stq_addr_virtual =  Output(Vec(numStqEntries,Bool()))
  
  val issued76 = Input(Bool())
  val exed76 = Input(Bool())
  val issued84 = Input(Bool())
  val issued92 = Input(Bool())
  val testissued = Input(Bool())
  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  io.dmem.st_risk_table <> io.core.st_risk_table
  io.dmem.st_fp_risk_table <> io.core.st_fp_risk_table
 ...
 // Prioritize emptying the store queue when it is almost full    //当store队列几乎满时，优先清空它
  ...
  for (w <- 0 until memWidth) {
    ...
    // STA UOP计算存储地址并更新STQ条目中的地址。STD UOP将存储数据移动到STQ条目中
  ...
  for(i <- 0 until numLdqEntries){
    io.core.ldq_dep_mask(i) := ldq(i).bits.st_dep_mask
    io.core.ldq_valid(i) := ldq(i).valid
  }
  for(i <- 0 until numStqEntries){
    io.core.stq_valid(i) := stq(i).valid
  ...
  for (w <- 0 until memWidth) {
    ...
    when(io.core.testissued){
      ...
      printf("ldq(6) = 0x%x ",ldq(6).bits.uop.debug_inst)
      printf("ldq-6-st_dep_mask = %d",ldq(6).bits.st_dep_mask)
      printf("find0 = 0x%x ",stq(0).bits.uop.debug_inst(31,0))
      printf("find1 = 0x%x ",stq(1).bits.uop.debug_inst(31,0))
      printf("find2 = 0x%x ",stq(2).bits.uop.debug_inst(31,0))
      printf("find3 = 0x%x ",stq(3).bits.uop.debug_inst(31,0))
      printf("find4 = 0x%x ",stq(4).bits.uop.debug_inst(31,0))
      printf("find5 = 0x%x ",stq(5).bits.uop.debug_inst(31,0))
      printf("find6 = 0x%x ",stq(6).bits.uop.debug_inst(31,0))
      printf("find7 = 0x%x ",stq(7).bits.uop.debug_inst(31,0))
      for (i <- 0 until numStqEntries) { //遍历stq，判断有无地址未解析的store
        when(!stq(i).bits.addr.valid && stq(i).valid) {
          printf(p"find addr_not_get when 84-testissued\n")
          printf(p"i=${i} ")
          printf("find = 0x%x ",stq(i).bits.uop.debug_inst(31,0))
          printf(p"find storeaddr_not_get = ${io.core.storeaddr_not_get} when 84-testissued\n")
        }
      }
    }
  
    when(io.core.issued84){
      printf(p"find st_dep_mask=${ldq(dmem_req(w).bits.uop.ldq_idx).bits.st_dep_mask} when 84-issued\n")
      printf("find0 = 0x%x ",stq(0).bits.uop.debug_inst(31,0))
      printf("find1 = 0x%x ",stq(1).bits.uop.debug_inst(31,0))
      printf("find2 = 0x%x ",stq(2).bits.uop.debug_inst(31,0))
      printf("find3 = 0x%x ",stq(3).bits.uop.debug_inst(31,0))
      ...
      for (i <- 0 until numStqEntries) { //遍历stq，判断有无地址未解析的store
        when(!stq(i).bits.addr.valid && stq(i).valid) {
          printf(p"find addr_not_get when 84-issued\n")
          printf(p"i=${i} ")
          printf("find = 0x%x ",stq(i).bits.uop.debug_inst(31,0))
          printf(p"find storeaddr_not_get = ${io.core.storeaddr_not_get} when 84-issued\n")
        }
      }
    }
  
    when(io.core.issued76){
      printf(p"find st_dep_mask=${ldq(dmem_req(w).bits.uop.ldq_idx).bits.st_dep_mask} when 76-issued\n")
      printf("ldq(6) = 0x%x ",ldq(6).bits.uop.debug_inst)
      printf("ldq-6-st_dep_mask = %d",ldq(6).bits.st_dep_mask)
      printf("find0 = 0x%x ",stq(0).bits.uop.debug_inst(31,0))
      printf("find1 = 0x%x ",stq(1).bits.uop.debug_inst(31,0))
      ...
      for (i <- 0 until numStqEntries) { //遍历stq，判断有无地址未解析的store
        when(!stq(i).bits.addr.valid && stq(i).valid) {
          printf(p"find addr_not_get when 76-issued\n")
          printf(p"i=${i} ")
          //printf("find—addr = 0x%x ",stq(i).bits.addr.bits)
          printf("find = 0x%x ",stq(i).bits.uop.debug_inst(31,0))
          printf(p"find storeaddr_not_get = ${io.core.storeaddr_not_get} when 76-issued\n")
        }
      }
    }

    when(io.core.exed76){
      printf(p"find st_dep_mask=${ldq(dmem_req(w).bits.uop.ldq_idx).bits.st_dep_mask} when 76-exed\n")
      printf("ldq(6) = 0x%x ",ldq(6).bits.uop.debug_inst)
      printf("ldq-6-st_dep_mask = %d",ldq(6).bits.st_dep_mask)
      printf("find0 = 0x%x ",stq(0).bits.uop.debug_inst(31,0))
      printf("find1 = 0x%x ",stq(1).bits.uop.debug_inst(31,0))
      ...
      for (i <- 0 until numStqEntries) { //遍历stq，判断有无地址未解析的store
        when(!stq(i).bits.addr.valid && stq(i).valid) {
          printf(p"find addr_not_get when 76-exed\n")
          printf(p"i=${i} ")
          //printf("find—addr = 0x%x ",stq(i).bits.addr.bits)
          printf("find = 0x%x ",stq(i).bits.uop.debug_inst(31,0))
          printf(p"find storeaddr_not_get = ${io.core.storeaddr_not_get} when 76-exed\n")
        }
      }
    }
  
    when(dmem_req(w).bits.uop.debug_inst(31, 0) === 0x00054783L.U) {
      printf(p"find st_dep_mask=${ldq(dmem_req(w).bits.uop.ldq_idx).bits.st_dep_mask} when 84\n")
      printf("ldq = 0x%d ",dmem_req(w).bits.uop.ldq_idx)
      printf("ldq(6) = 0x%x ",ldq(6).bits.uop.debug_inst)
      printf("ldq-6-st_dep_mask = %d",ldq(6).bits.st_dep_mask)
      printf("find0 = 0x%x ",stq(0).bits.uop.debug_inst(31,0))
      printf("find1 = 0x%x ",stq(1).bits.uop.debug_inst(31,0))
      ...
      for (i <- 0 until numStqEntries) { //遍历stq，判断有无地址未解析的store
        when(!stq(i).bits.addr.valid && stq(i).valid) {
          printf(p"find addr_not_get when 84\n")
          printf(p"i=${i} ")
          //printf("find—addr = 0x%x ",stq(i).bits.addr.bits)
          printf("find = 0x%x ",stq(i).bits.uop.debug_inst(31,0))
          printf(p"find storeaddr_not_get = ${io.core.storeaddr_not_get} when 84\n")
        }
      }
    }
