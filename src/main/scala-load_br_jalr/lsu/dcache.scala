  ...
  with HasBoomCoreParameters
{
  ...
  // Miss handling
  for (w <- 0 until memWidth) {
    val pdst_risk = Mux(s2_req(w).uop.dst_rtype === RT_FIX, io.lsu.risk_table(s2_req(w).uop.pdst), Mux(s2_req(w).uop.dst_rtype === RT_FLT, io.lsu.fp_risk_table(s2_req(w).uop.pdst), false.B) )
    val prs1_risk = Mux(s2_req(w).uop.lrs1_rtype === RT_FIX, io.lsu.risk_table(s2_req(w).uop.prs1), Mux(s2_req(w).uop.lrs1_rtype === RT_FLT, io.lsu.fp_risk_table(s2_req(w).uop.prs1), false.B) )
    val prs2_risk = Mux(s2_req(w).uop.lrs2_rtype === RT_FIX, io.lsu.risk_table(s2_req(w).uop.prs2), Mux(s2_req(w).uop.lrs2_rtype === RT_FLT, io.lsu.fp_risk_table(s2_req(w).uop.prs2), false.B) )
    val prs3_risk = Mux(s2_req(w).uop.frs3_en, io.lsu.fp_risk_table(s2_req(w).uop.prs3), false.B)
    val pdst_risk_st = Mux(s2_req(w).uop.dst_rtype === RT_FIX, io.lsu.st_risk_table(s2_req(w).uop.pdst), Mux(s2_req(w).uop.dst_rtype === RT_FLT, io.lsu.st_fp_risk_table(s2_req(w).uop.pdst), false.B) )
    val prs1_risk_st = Mux(s2_req(w).uop.lrs1_rtype === RT_FIX, io.lsu.st_risk_table(s2_req(w).uop.prs1), Mux(s2_req(w).uop.lrs1_rtype === RT_FLT, io.lsu.st_fp_risk_table(s2_req(w).uop.prs1), false.B) )
    val prs2_risk_st = Mux(s2_req(w).uop.lrs2_rtype === RT_FIX, io.lsu.st_risk_table(s2_req(w).uop.prs2), Mux(s2_req(w).uop.lrs2_rtype === RT_FLT, io.lsu.st_fp_risk_table(s2_req(w).uop.prs2), false.B) )
    val prs3_risk_st = Mux(s2_req(w).uop.frs3_en, io.lsu.st_fp_risk_table(s2_req(w).uop.prs3), false.B)
  ...
  when (mshrs.io.req.map(_.fire()).reduce(_||_)) {                   //cache匹配只需要两个周期
    ...
    for(w <- 0 until memWidth) {
      ...
      when(s2_req(w).uop.debug_inst === 0x0007c783L.U) {
        printf(p" pdst=${s2_req(w).uop.pdst} ")
        printf(p" pdst=${s2_req(w).uop.prs1} ")
        printf(p" pdst=${s2_req(w).uop.prs2} ")
        printf(p" mask=${s2_req(w).uop.br_mask} ")
        printf(p" prs1-risk=${io.lsu.risk_table(s2_req(w).uop.prs1)} ")
        printf(p" prs2-risk=${io.lsu.risk_table(s2_req(w).uop.prs2)} ")
        printf(p" pdst-risk=${io.lsu.risk_table(s2_req(w).uop.pdst)} ")
