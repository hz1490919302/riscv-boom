  ...
  extends IssueUnit(params.numEntries, params.issueWidth, numWakeupPorts, params.iqType, params.dispatchWidth)
{
  ...
  for (i <- 0 until numIssueSlots) {
    ...
    for (w <- 0 until issueWidth) {
      ...
      when(issue_slots(i).uop.debug_pc === 0x80001d7aL.U){
        printf(p" cycles=${io.idle_cycles} ")
        printf(p" requests(i)=${requests(i)} ")
        printf(p" !uop_issued=${!uop_issued} ")
        printf(p" can_allocate=${can_allocate} ")
        printf(p" !port_issued(w)=${!port_issued(w)} ")
        printf(" find 0x80001d7aL \n")
      }
