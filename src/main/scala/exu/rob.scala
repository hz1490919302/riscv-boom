  ...
  )(implicit p: Parameters)  extends BoomBundle
{
  ...
  //val wakeup_yes = Input(Vec(numRobRows,Bool()))
 // val wakeup_already = Output(Vec(numRobRows,Bool()))
 // val wakeup_valid = Output(Vec(numRobRows,Bool()))
  //val wakeup_pdst = Output(Vec(numRobRows,UInt(32.W)))
  //val wakeup_debug_inst = Output(Vec(numRobRows,UInt(32.W)))
  //val wakeup_uop_dst_rtype  = Output(Vec(numRobRows, UInt(2.W)))
  val rob_mask = Output(Vec(numRobRows,UInt(32.W)))
  val rob_val = Output(Vec(numRobRows,Bool()))
  val rob_debug_pc = Output(Vec(numRobRows,UInt(32.W)))
  ...
  val clear_st_risk_table = Output(Vec(numIntPhysRegs+numFpPhysRegs,UInt(1.W)))  //10 is random
  val return_issue = Output(Bool())
  val ldq_dep_mask = Input(Vec(numLdqEntries,UInt(numLdqEntries.W)))
  val stq_valid = Input(Vec(numStqEntries,Bool()))
  val stq_addr_valid = Input(Vec(numStqEntries,Bool()))
  val stq_addr_virtual =  Input(Vec(numStqEntries,Bool()))

  val risk_table = Input(Vec(numIntPhysRegs,Bool()))
  val fp_risk_table = Input(Vec(numFpPhysRegs,Bool()))
  val st_risk_table = Input(Vec(numIntPhysRegs,Bool()))
  val st_fp_risk_table = Input(Vec(numFpPhysRegs,Bool()))
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    /*val wakeup_row = RegInit(VecInit(Seq.fill(32){0.U(1.W)}))
    val wakeup_already = RegInit(VecInit(Seq.fill(32){false.B}))
    val wakeup_valid = RegInit(VecInit(Seq.fill(32){false.B}))*/
    

    for(i <- 0 until numRobRows) {
      io.rob_mask(i) := rob_uop(i).br_mask
      io.rob_val(i) := rob_val(i)
      io.rob_debug_pc(i) := rob_uop(i).debug_pc
      io.rob_uop(i) := rob_uop(i)

      //io.wakeup_already(i) := wakeup_already(i)
      //io.wakeup_valid(i)  := wakeup_valid(i)
      //io.wakeup_pdst(i)  := rob_uop(i).pdst
      //io.wakeup_debug_inst(i) := rob_uop(i).debug_inst
      //io.wakeup_uop_dst_rtype(i) := rob_uop(i).dst_rtype
    }

    val clear_risk_table = RegInit(VecInit(Seq.fill(numIntPhysRegs+numFpPhysRegs){0.U(1.W)}))   //10 is random
    val clear_st_risk_table = RegInit(VecInit(Seq.fill(numIntPhysRegs+numFpPhysRegs){0.U(1.W)}))   //10 is random
    for(i <- 0 until numIntPhysRegs+numFpPhysRegs) {
      io.clear_risk_table(i) := clear_risk_table(i)
      io.clear_st_risk_table(i) := clear_st_risk_table(i)
    }
    var fflag = 0.U(32.W)
    var fflag1 = 0.U(32.W)
    var fflagx = 0.U(32.W)

       //跟踪风险指令 rob,解除后通知core重新该pdst重新去唤醒其他的指令
      /*for(j <- 0 until numRobRows) {

        val load_can_clear = (0 until numStqEntries).map{ k => Mux(io.ldq_dep_mask(rob_uop(j).ldq_idx)(k) === 1.U ,io.stq_valid(k) && io.stq_addr_valid(k) && !io.stq_addr_virtual(k), true.B)}.reduce(_&&_)
        //load解除污染
        val no_load_can_clear = (0 until numStqEntries).map{ k => Mux(io.ldq_dep_mask((rob_uop(j).ldq_idx+7.U)%8.U)(k) === 1.U ,io.stq_valid(k) && io.stq_addr_valid(k) && !io.stq_addr_virtual(k), true.B)}.reduce(_&&_)

        when(  rob_val(j) =/= 0.U && ( (rob_uop(j).uses_ldq && load_can_clear) || (!rob_uop(j).uses_ldq && no_load_can_clear) )
          && wakeup_row(j) =/= 0.U &&  rob_uop(j).br_mask === 0.U  && !wakeup_already(j) ) {  //等risk消除后,发出该唤醒 //
        io.wakeup_valid(j) := 1.U
        io.wakeup_already(j) := 1.U
        //wakeup_valid := (1.U << j) | wakeup_valid
        //wakeup_already := (1.U << j) | wakeup_already
        wakeup_valid(j) := 1.U
        wakeup_already(j) := 1.U
        wakeup_row(j) := 0.U

        printf(p" wakeup_row(j)=${wakeup_row(j)} ")
        printf("  found-risk-debug-inst2 wakeup_inst(j)=0x%x", rob_uop(j).debug_inst)
        printf(p" j=${j} ")
        printf(p" cycles=${io.idle_cycles} ")
        printf(p" \n")
      }
    ...
    for(i <- 0 until numRobRows) {
      ...
      val load_can_clear = (0 until numStqEntries).map{ j => Mux(io.ldq_dep_mask(rob_uop(i).ldq_idx)(j) === 1.U ,io.stq_valid(j) && io.stq_addr_valid(j) && !io.stq_addr_virtual(j), true.B)}.reduce(_&&_)
      //load解除污染
      val no_load_can_clear = (0 until numStqEntries).map{ j => Mux(io.ldq_dep_mask((rob_uop(i).ldq_idx+7.U)%8.U)(j) === 1.U ,io.stq_valid(j) && io.stq_addr_valid(j) && !io.stq_addr_virtual(j), true.B)}.reduce(_&&_)
      //非load解除污染
      when(rob_val(i) =/= 0.U && ( (rob_uop(i).uses_ldq && load_can_clear) || (!rob_uop(i).uses_ldq && no_load_can_clear) ) && ((rob_uop(i).dst_rtype === RT_FIX && io.st_risk_table(rob_uop(i).pdst) === true.B) || (rob_uop(i).dst_rtype === RT_FLT && io.st_fp_risk_table(rob_uop(i).pdst) === true.B)) && rob_uop(i).pdst =/= 0.U){     // TODO

        //clear_st_risk_table(fflag1) := Mux(rob_uop(i).dst_rtype === RT_FLT,rob_uop(i).pdst + numIntPhysRegs.U,rob_uop(i).pdst) //通知st_risk_table消除该pdst的risk位
        val index_pdst = Mux(rob_uop(i).dst_rtype === RT_FLT,rob_uop(i).pdst + numIntPhysRegs.U,rob_uop(i).pdst)
        ...
        /*printf(p" start mask=${rob_uop(i).br_mask} ")
        printf(" inst=0x%x ", rob_uop(i).debug_inst)
        printf(" debug_pc=0x%x",rob_uop(i).debug_pc)
        printf(p" i=${i} ")
        printf(p" rob_val=${rob_val(i)} ")
        printf(p" cycles=${io.idle_cycles} ")
        printf(p" pdst_clear=${rob_uop(i).pdst} ")
        printf(p" fflag1=${fflag1} ")
        printf(p" load_can_clear=${rob_uop(i).uses_ldq && load_can_clear} ")
        printf(p" no_load_can_clear=${!rob_uop(i).uses_ldq && no_load_can_clear} ")
        printf(p" clear st risk table\n ")*/
    ...
    for(i <- 0 until numIntPhysRegs+numFpPhysRegs) {
      ...
      when(clear_st_risk_table(i) =/= 0.U){
        clear_st_risk_table(i) := 0.U
      }
    }
    

    //当推测性load解除推测性，并且已经唤醒/广播后,将该load在rob中的唤醒项清除
    //val wakeup_yes_core = Wire(Vec(numRobRows,Bool()))
    /*for(i <- 0 until numRobRows) {
      /*wakeup_yes_core(i) := io.wakeup_yes(i)
      when(wakeup_already(i) && wakeup_valid(i) && io.wakeup_yes(i)) {
        wakeup_already(i) := false.B
        wakeup_valid(i) := false.B
        //io.wakeup_already(i) := false.B
        //io.wakeup_valid(i) := false.B
        wakeup_row(i) := 0.U
        printf(p" \nrob_idx=${i} ")
        printf(p" cycles=${io.idle_cycles} ")
        printf(" found-risk-debug-inst3 ")
        printf(" rob_wakeup_valid_clear \n")
        //fflagx = (fflagx - 1.U + 4.U)%4.U
      }*/

      when(wakeup_already(i) && wakeup_valid(i)) {
        wakeup_already(i) := false.B
        wakeup_valid(i) := false.B
      }
   ...
   /* val return_issue  = Reg(Bool())     //不该是rob.numWakeupPorts+10
    ...
    //rob等待risk信号,来自issue-slot , br/jalr come from intIssue
   /* for(i <- 0 until intIssueParam.numEntries) {
      when(io.risk_pdst(i) =/= 0.U) {
        val is_fp = rob_uop(io.risk_rob_idx(i)).dst_rtype === RT_FLT
        printf(" found-risk-debug-inst1 ")
        printf(p" io.risk_pdst=${io.risk_pdst} ")
        printf(p" io.risk_pdst(i)=${io.risk_pdst(i)} ")
        printf(" risk_debug_inst=0x%x",io.risk_debug_inst(i))
        printf(" risk_debug_pc=0x%x",rob_uop(io.risk_rob_idx(i)).debug_pc)
        printf(p" cycles=${io.idle_cycles} ")
        printf(p" i=${i} ")
        printf(p" rob_val=${rob_val(io.risk_rob_idx(i))} ")
        printf(p" prs1=${rob_uop(io.risk_rob_idx(i)).prs1} ")
        printf(p" prs2=${rob_uop(io.risk_rob_idx(i)).prs2} ")
        printf(p" prs3=${rob_uop(io.risk_rob_idx(i)).prs3} ")
        printf(p" dst_type=${rob_uop(io.risk_rob_idx(i)).dst_rtype} ")
        printf(p" ldq=${rob_uop(io.risk_rob_idx(i)).uses_ldq} ")
        printf(p" st=${io.st_risk_table(io.risk_pdst(i)) } ")
        printf(p" not-st=${io.risk_table(io.risk_pdst(i)) } ")
        printf(p" br_mask=${rob_uop(io.risk_rob_idx(i)).br_mask} ")
        printf(p" wakeup_pdst=${rob_uop(io.risk_rob_idx(i)).pdst} ")
        //printf(p" wakeup_row=${wakeup_row} ")
        printf(p" !return_issue=${!return_issue} ")
        printf(p" io.risk_rob_idx=${io.risk_rob_idx} ")
        printf(p" io.risk_rob_idx=${io.risk_rob_idx(i)} \n")
         ...
         //不应该是numWakeupPorts+10,暂时图方便
          when( wakeup_row(io.risk_rob_idx(i)) === 0.U) {
            when(rob_uop(io.risk_rob_idx(i)).dst_rtype === 2.U ){
              printf(" \nfound inst =0x%x", rob_uop(io.risk_rob_idx(i)).debug_inst)
              printf(" found pc =0x%x", rob_uop(io.risk_rob_idx(i)).debug_pc)
              printf(p" \nfound dst_rtype = 2\n ")
            }
            //wakeup_pdst := (1.U << (io.risk_pdst(i))(8,0)) | wakeup_pdst
            //wakeup_row := ( 1.U << (io.risk_rob_idx(i)(8,0)) ) | wakeup_row
            val tt = io.risk_rob_idx(i)
            wakeup_row(tt) := 1.U
            printf(p" i=${i} ")
            printf(p" io.risk_pdst(i)-before=${io.risk_pdst(i)} ")
            printf(p" wakeup_row=${wakeup_row} \n")
            return_issue := true.B
          }
...
//                  "FP instruction writing back exc bits is overriding an existing exception.")
  ...
  when(io.flush.valid){
      ...
      printf(p"!is_mini_exception : =${!is_mini_exception} \n")
      printf(p"flush_commit : =${flush_commit} \n")
      val xx = flush_uop.uopc === uopERET
      printf(p"flush_uop.uopc === uopERET : =${xx} \n")
      printf(p"flush_val : =${flush_val} \n")
      printf(p"refetch_inst : =${refetch_inst} \n")
      
      //printf("io.commit.uops.pc=0x%x\n", io.commit.uops.debug_pc)
  }
