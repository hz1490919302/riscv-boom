  ...
  with IssueUnitConstants
{
  ...
  for (i <- 0 until numWakeupPorts) {
    when (io.wakeup_ports(i).valid &&
      ...
      when(io.wakeup_ports(i).bits.risk =/= 0.U && io.wakeup_ports(i).bits.pdst =/= 0.U && next_uop.uses_ldq){
      //when(io.wakeup_ports(i).bits.risk =/= 0.U && io.wakeup_ports(i).bits.pdst =/= 0.U && next_uop.uses_ldq && io.wakeup_ports(i).bits.debug_inst(15,0) === 0x97baL.U && next_uop.debug_inst === 0x0007c783L.U){
       /* io.risk_pdst := io.wakeup_ports(i).bits.pdst
        ...
        printf(" risk_debug_inst:(0x%x)", io.wakeup_ports(i).bits.debug_inst(15,0))
    ...
    when (io.wakeup_ports(i).valid &&
      ...
      //  when(io.wakeup_ports(i).bits.risk =/= 0.U && io.wakeup_ports(i).bits.pdst =/= 0.U && next_uop.uses_ldq && io.wakeup_ports(i).bits.debug_inst(15,0) === 0x97baL.U && next_uop.debug_inst === 0x0007c783L.U){
        ...
        io.risk_debug_inst := io.wakeup_ports(i).bits.debug_inst
        io.risk_rob_idx := io.wakeup_ports(i).bits.risk_rob_idx
        printf(p" risk_pdst=${io.risk_pdst} ")
        printf(" risk_debug_inst:(0x%x)", io.risk_debug_inst)
        printf(p" cycles=${io.idle_cycles} ")
...
/*  when(slot_uop.debug_inst(15,0) === 0x97baL.U){
      printf(" wakeup0:(0x%x) ", io.wakeup_ports(0).bits.debug_inst)
      printf(p" mask0=${io.wakeup_ports(0).bits.br_mask =/= 0.U} ")
      printf(" wakeup1:(0x%x) ", io.wakeup_ports(1).bits.debug_inst)
      printf(p" p1=${p1} ")
      printf(p" p2=${p2} ")
      printf(p" is_valid=${is_valid} ")
      printf(p" request=${io.request} \n")
  }*/
